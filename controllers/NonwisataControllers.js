...
export const getNonwisata = async (req, res) => {
  const connection = await query();
  try {
    const [rows] = await connection.execute('SELECT * FROM nonwisata');
    res.json(rows);
  } catch (error) {
...
export const getNonwisataById = async (req, res) => {
  const connection = await query();
  try {
    const [rows] = await connection.execute('SELECT * FROM nonwisata WHERE id = ?', [req.params.id]);
...
export const saveNonwisata = async (req, res) => {
  try {
    const {
      nama_tempat,
      deskripsi_singkat,
    ...
    const connection = await query();
      ...
      await connection.execute(
        'INSERT INTO nonwisata (nama_tempat, deskripsi_singkat, alamat, link_maps, deskripsi_panjang, jam_buka, ket1, harga1, cover, foto1, foto2, foto3, foto4, foto5 ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ',
        [
        nama_tempat, deskripsi_singkat, alamat, link_maps, deskripsi_panjang, jam_buka,
        ket1, harga1,
...
export const deleteNonwisata = async (req, res) => {
  try {
    const { id } = req.params; // Ambil ID dari parameter URL
    
    const connection = await query();
    // Dapatkan file paths dari database berdasarkan ID
    const [rows]  = await connection.execute('SELECT cover, foto1, foto2, foto3, foto4, foto5 FROM nonwisata WHERE id = ?', [id]);
    ...
    await connection.execute('DELETE FROM nonwisata WHERE id = ?', [id]);
